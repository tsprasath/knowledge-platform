...
object AssessmentManager {
	...
	val map = Map("contentId" -> "contentId", "batchId" -> "batchId", "userId" -> "userId", "attemptId" -> "attemptId")
	...
	def validateAssessRequest(req: Request) = {
		...
		questToken.get(AssessmentConstants.QUESTION_LIST).asInstanceOf[util.List[String]]
	}

	def questionList(fields: util.List[String]): Response = {
